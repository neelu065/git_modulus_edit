...
def phi_evaluation_neighbor(self, x_values, y_values):
    ...
    alpha = torch.distributions.uniform.Uniform(- temp_angle, temp_angle).sample(
        [self.cfg.custom.neigh_point_kd_tree, 1]).cuda().type(torch.float32)
    alpha_xy = torch.distributions.uniform.Uniform(- temp_angle, temp_angle).sample([1]).cuda().type(
        torch.float32)
    ...
    phi_xby = self.nets.evaluate({'x': x_values[1], 'y': y_values[1], 'alpha': alpha})[
        'phi']  # We evaluate the phi values of the neighbors of xby using the neural network.
    phi_xyf = self.nets.evaluate({'x': x_values[2], 'y': y_values[2], 'alpha': alpha})[
        'phi']  # We evaluate the phi values of the neighbors of xyf using the neural network.
    phi_xyff = self.nets.evaluate({'x': x_values[3], 'y': y_values[3], 'alpha': alpha})[
        'phi']  # We evaluate the phi values of the neighbors of xyff using the neural network
    xy_pred = self.nets.evaluate({'x': x_values[4], 'y': y_values[4], 'alpha': alpha_xy})
...
def init_domain(interior_invar, domain_invar, cfg=ModulusConfig):
    ...
    for i in range(2):
        ...
        def neigh_weigh_dist(j):
            ...
            # We add the neighbors to the neighbors list.
            distnace = [dist_xfy, dist_xby, dist_xyf, dist_xyff]
            # all distances are appended to the distance list.
...
class PotentialLoss(Loss):
    ...
    def in_belt_residuals(self):
        ...
        def residuals_calc(i):
            ...
            # xby
            x_xby = torch.stack(self.neighbors[i][1])[:, [0]].cuda().type(torch.float32)
            y_xby = torch.stack(self.neighbors[i][1])[:, [1]].cuda().type(torch.float32)

            # xyf
            x_xyf = torch.stack(self.neighbors[i][2])[:, [0]].cuda().type(torch.float32)
            y_xyf = torch.stack(self.neighbors[i][2])[:, [1]].cuda().type(torch.float32)

            # xyff
            x_xyff = torch.stack(self.neighbors[i][3])[:, [0]].cuda().type(torch.float32)
            y_xyff = torch.stack(self.neighbors[i][3])[:, [1]].cuda().type(torch.float32)

            # xy
            xy = torch.reshape(torch.hstack([self.neighbors[i][4][0], self.neighbors[i][4][1]]), (2, 1)).cuda().type(
    ...
    def forward(self, invar, pre_outvar, true_outvar, lambda_weigh, step: int):
        ...
        # index_belt_interior = self.belt_total.unsqueeze(1).eq(total_interior).all(-1).any(-1)
